...
## Function definition
fcv.cov.yx2.var.x2 <- function( icc.y, icc.x, b2, b1, k, n ){
		var.y2 <- icc.y
		var.y1 <- 1 - var.y2
		var.x2 <- icc.x
		var.x1 <- 1 - icc.x
		cov.yx2 <- b2*var.x2
		cov.yx1 <- b1*var.x1	
		cv.cov.yx2.var.x2 <- ((2*(n*cov.yx2+cov.yx1)*(n*var.x2+var.x1))/(k-1)+(2*cov.yx1*var.x1)/((n-1)*k))/(n^2)
		return( cv.cov.yx2.var.x2 )
}
